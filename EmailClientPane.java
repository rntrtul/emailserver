		...
		dateTime = date.toString();
                ...
                if (whatId == Globals.RECEIVER_ID) {
                    messageHeaders[i] = Globals.boxMessages[i].substring(Globals.SENDER_POS, Globals.RECEIVER_POS) + "  " +
                                        dateTime + "  " +
                                        Globals.boxMessages[i].substring(Globals.FIRST_RECORD_MARKER_POS + 1, Globals.boxMessages[i].indexOf(Globals.END_OF_SUBJECT_MARKER));
                }
                else {
...
// there is a bug here: when the message is empty, the substring returns -1, then +1 is added and so an empty string is returned
	...
	messageText = Globals.boxMessages[itemIndex].substring(Globals.boxMessages[itemIndex].indexOf(Globals.END_OF_SUBJECT_MARKER) + 1);
